...
Open the file [Math 221 Statistics Toolbox](https://byuistats.github.io/BYUI_M221_Book/Data/Math221StatisticsToolbox.xlsx) and do the following:
...
- To determine if the data are normally distributed, we will look at a histogram plot for each group separately.
    + While still on the ANOVA tab, scroll to the right. You should see a histogram. In a large, yellow highlighted box directly above the plot you can type the group number of the sample you would like to see a histogram for (see the picture below). Make sure that you evaluate each sample's data. They **all** must look *approximately normal* to meet the requirement.
...
The summary statistics can be obtained using the [Math 221 Statistics Toolbox](https://byuistats.github.io/BYUI_M221_Book/Data/Math221StatisticsToolbox.xlsx).  Paste the three columns of data into the appropriate areas of column A, B, and C.  The summary statistics will appear in the table to the right labeled "Numerical Summary of the Data".
...
The points in the histograms for each of the three groups (not shown) **follow the line closely**, suggesting that the data can be assumed to be normally distributed. 
