...
- The test statistic, $F$, is given in cell V7
- The degrees of freedom are presented in cells T7 and T8
- The $P$-value is reported in cell W7
...
- To determine if the data are normally distributed, we will look at a Q-Q plot for each group separately.
    + While still on the ANOVA tab, scroll to the right so that columns Y through AI are visible. You should see a Q-Q plot. In a yellow highlighted drop-down box directly above the plot you can select the sample you would like to see a Q-Q plot for (see the picture below). Make sure that you evaluate each sample's data. They all must look approximately normal to meet the requirement.
  
  <img src="./Images/ANOVA_QQplot.PNG">
...
Side-by-side box plots are an excellent way to display data from multiple samples.

<img src="./Images/Gratitude-Boxplots.png">

The Math221 Statistics Toolbox Excel file also produces a means plot graph just below the numerical summary. This plot is acceptable, but unfortunately, from this plot you don't get an idea about the spread of the data like you do with a boxplot.

<img src="./Images/ANOVA_MeanPlot.PNG">
...
The points in the Q-Q plots for each of the three groups (not shown) follow the line closely, suggesting that the data can be assumed to be normally distributed. 
