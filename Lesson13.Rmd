...
Use the "Two-sample t Test" tab of the [Math 221 Statistics Toolbox](./Data/Math221StatisticsToolbox.xlsx) to create separate histograms, one for each group.  

<img src="./Images/ReadingPractices-two-Histograms-Excel.png">
...
**To create the side-by-side boxplots in Excel:** 
In this case it is easiest to switch the order of the columns. Put HeartAttacks in the column to the *right* of TimePeriod. Then highlight both columns (including the column headers), and create the boxplot with these steps:
...
Remember, to make side-by-side boxplots the group label must be in the left column and the data values in the right column. The data for this example is already arranged this way. So, to create the side-by-side boxplots just highlight both columns and then follow the steps for creating a single boxplot: click the Insert ribbon, then click the Histogram icon and choose the image of side-by-side boxplots under the "Box and Whisker" heading.
...
<br>
<br>
This boxplot was made by highlighting both columns of data and then following the normal steps for creating a boxplot in Excel. Adding a legend makes the graph easier to understand.
</div>
<br>
