...
3. Of the four types of confidence intervals listed below, which one is appropriate for this experiment?
    ...
    a. Confidence interval for $\mu$ when $\sigma$  is known.
    b. Confidence interval for $\mu$ when $\sigma$  is unknown.
...
6. To assess the accuracy of a laboratory scale, a standard weight that is known to weigh 1 gram is repeatedly weighed a total of n times and the mean of the weighings is computed. Suppose the scale readings are normally distributed with unknown mean $\mu$ and standard deviation $\sigma$  = 0.01 g.  How large should n be so that a 95% confidence interval for $\mu$ has a margin of error of ± 0.0001? 
...
7. The mean area $\mu$ of the several thousand apartments in a new development is advertised to be 1250 square feet.  A tenant group thinks that the apartments are smaller than advertised.  They hire an engineer to measure a random sample of apartments to test their suspicion.  The appropriate null and alternative hypotheses, $H_0$ and $H_a$, for $\mu$ are 
	
    a. $H_0: \mu=1250$          $H_a: \mu<1250$  
    b. $H_0: \mu=1250$          $H_a: \mu≠1250$
    c. $H_0: \mu=1250$          $H_a: \mu>1250$
    d. Cannot specify without knowing the size of the sample used by the engineer

<br>

8. In a statistical test of hypotheses, we say the test result is statistically significant at the level of significance ($\alpha$) if 
	
    a. $\alpha$  = 0.05
    b. the $P$-$\text{value}$ is less than $\alpha$  
    c. $\alpha$  is small
    d. the $P$-$\text{value}$ is larger than $\alpha$ 
...
11. If the level of significance, $\alpha$ , is made very small, thereby making the probability of committing a Type 1 error very small, what happens to the probability of committing a Type 2 error?
