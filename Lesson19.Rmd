...
A table of expected counts is found starting in cell U4 (scroll to the right of the data entry area and hypothesis test results). If one of the expected counts is less than 5 the cell will be red, otherwise they all remain gray as seen here:

<img src="./Images/Excel-Chi-Req-Expected-Counts.png">

<br>

Let's walk through the chiropractice example now from beginning to end: <!--[To do: Flesh this out a bit]:-->
...
There may be no general guideline for deciding which variable is the row variable, but the graphics produced by your software may depend on this decision. For example, Excel will give you a different clustered bar chart when you use "Location" as the row variable than when you use "Motivation" as the row variable. Sometimes, choosing one of the variables as the row variable makes the clustered bar chart easier to understand. The bar chart is just just below the data entry area in Excel.
