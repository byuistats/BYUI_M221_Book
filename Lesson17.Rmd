...
The point estimate for $p$ is $\hat p$.  You might want to review the mean and standard deviation of the random variable $\hat p$ in the lesson on [Describing Categorical Data: Proportions; Sampling Distribution of a Sample Proportion](Lesson16.html){target="_blank"}. Traditionally, people have used these equations to create confidence intervals for the population proportion.
...
- The yellow boxes indicate the input spaces. The first step, getting an estimate of $\hat{p}$, is accomplished by inputting the value of $x$ and the value of $n$ from the data above. Look at the group of cells labeled "Numerical Summary" and input $x$ into cell D7. Type $n$ in cell E7. (You can ignore column A which asks you to paste in data. We will only be putting in summary data of counts, not individual observations.)  
...
- **Step 2**: Next we must indicate our desired level of confidence in cell J11. By default it is set to 0.95, meaning a 95% confidence level. The 95% confidence interval is given in the cells L14 and L15. For convenience, $\hat{p}$ and the margin of error are also shown in this block of output.
...
<br>
<div class="message Tip">**Review:**  For a review of how to make pie graphs and bar charts in Excel, read [Describing Categorical Data: Proportions; Sampling Distribution of a Sample Proportion](Lesson16.html){target="_blank"}</div>
<br>
<br>
